...
var knownCommands = require('./commands').commands;
...
_.each(knownCommands, function(fn, name) {
  DebugClient.prototype[name] = function(opts, cb) {
    if ('function' === typeof opts) {
      cb = opts;
      opts = undefined;
    }
    return fn.call(this, opts).nodeify(cb);
  };
});
...
DebugClient.prototype.updateRunning = function(running) {
  ...
  if (running !== this.running) {
    ...
    debug('running: %s', running);
  }
};

DebugClient.prototype._onIncoming = function (incoming) {
  /*jshint eqnull:true */
  if (incoming.running != null)
    this.updateRunning(incoming.running);
};

DebugClient.prototype._onEvent = function(e) {
  var event = e.event;
  if (event === 'break' || event === 'exception') {
    this.backtrace(function(err, breakEvent) {
      breakEvent.reason = event;
      this.emit('paused', breakEvent);
    }.bind(this));
  }
  this.emit(e.event, e);
  this.emit('event', e);
